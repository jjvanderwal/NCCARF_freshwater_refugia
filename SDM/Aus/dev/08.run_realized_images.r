...
current=paste('/home/jc148322/NARPfreshwater/SDM/models/',spp,'/output/realized/current.csv',sep='')
...
files=list.files(paste(spp.dir,'output/realized/',sep=''))
files=files[-grep('realized.clip.csv',files)]

for (tfile in files) {
	filename=gsub('.csv','',tfile)

	spdata=read.csv(paste(spp.dir,'output/realized/',tfile,sep=''),as.is=T)
	...
	png(paste(out.dir,filename,'.realized.png',sep=''),width=dim(base.asc)[1], height=dim(base.asc)[2], units='px', pointsize=20, bg='white')
