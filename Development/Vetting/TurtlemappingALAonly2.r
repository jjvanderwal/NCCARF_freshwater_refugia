...
for (sp in 1:length(UCTurtles)) { 
          ...
          DECLAT = as.numeric(as.character(substr((str_split_fixed(tdata$V6, fixed(","), 2)[, 1]),1,nchar(str_split_fixed(tdata$V6, fixed(","), 2)[, 1])-1)))
          ...
          spfilenew=as.data.frame(spfile)
...
for (sp in 1:length(species)) { cat(species[sp],'\n')
              ...
              assign.list(min.lon,max.lon,min.lat,max.lat) %=% dynamic.zoom(species.data.ala$LONGDEC,species.data.ala$LATDEC, padding.percent=10)
        ...
        if (max.lat>=-18 & min.lat<=-34 |
              ...
              png(paste(gsub(".csv","",speciesname),'.png',sep=''),width=dim(base.asc)[1]*2+30, height=dim(base.asc)[1]*2+80, units='px', pointsize=20, bg='white')
                              ...
                              legend = c("ALA records", "CTBCC records"),
                              bty = "n", pch = c(19, 19),           # bty = "n": no box
                              col = c(2, 3), pt.cex = c(4, 4),
                              lty = c(-1, -1), cex=3)
              dev.off()
