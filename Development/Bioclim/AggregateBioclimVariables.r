################################################################################
# aggregate bioclim variables generated at 5km resolution onto Janets reaches for futures
...
curdir = paste(out.dir,"current.1976to2005",sep=''); dir.create(curdir) #define and create output directory	  
	...
	for(bios in bioclims) { cat(bios,'\n') 					# cycle through each bioclim variable
			tasc = read.asc.gz(paste(curdir,"/",bios,sep='')) # bring in relevant bioclim asci
			...
			system(paste('gzip ',curdir,'*.asc',sep='')) #gzip the data

	}

	
#### aggregate each bioclim variable to reach for current 1960 to 1990
